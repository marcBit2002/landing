...
function selectUser($mail)
{
    try {
        ...
        $_SESSION['incorrectLogin'] =  errorMessage($e);
    }
    $db = Disconnect();
    return $result;
}
...
function NouRegistre($nombre, $correo, $contrasenia)
{
    try {
        ...
        $query = "INSERT INTO USUARIOS (nombre, correo, contrasenia, idTipo) VALUES ('$nombre', '$correo', '$hash', 3)";
        $sentence = $db->prepare($query);
        $sentence->execute();
...
function NouRegistreAdministracio($nombre, $correo, $contrasenia, $tipo)
{
    try {
        $db = Connect();
        $hash = password_hash($contrasenia, PASSWORD_DEFAULT);
        $query = "INSERT INTO USUARIOS (nombre, correo, contrasenia, idTipo) VALUES ('$nombre', '$correo', '$hash', $tipo)";
        $sentence = $db->prepare($query);
        $sentence->execute();
...
function ModificarUsuarios($id, $nombre, $correo, $password, $tipo)
{
    try {
        ...
        $query = "UPDATE usuarios set nombre ='$nombre', correo='$correo', contrasenia='$hash', idTipo='$tipo' where id = $id ";
        $stmt = $db->prepare($query);
        $stmt->execute();
